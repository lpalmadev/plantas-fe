...
import { CreatePlantGenusDTO } from "../../lib/plant-genus/types.ts";

interface PlantGenusCreateModalProps {
    open: boolean;
    onClose: () => void;
    onSubmitSuccess?: (data: CreatePlantGenusDTO) => void;
    isDark?: boolean;
}

export function PlantGenusCreateModal({
                                          open,
                                          onClose,
                                          onSubmitSuccess,
                                          isDark = false
                                      }: PlantGenusCreateModalProps) {
    const {
        register,
        handleSubmit,
        reset,
        formState: { errors, isSubmitting },
    } = useForm<CreatePlantGenusDTO>();

    const onSubmit = async (data: CreatePlantGenusDTO) => {
        if (onSubmitSuccess) {
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/40">
            <div className={`${isDark ? 'bg-gray-800 text-white' : 'bg-white text-black'} rounded-2xl shadow-2xl w-full max-w-md p-4 relative`}>
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-bold">Crear nuevo género</h2>
                    <button
