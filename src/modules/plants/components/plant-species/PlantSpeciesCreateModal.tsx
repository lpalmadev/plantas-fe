...
import { CreatePlantSpeciesDTO } from "../../lib/plant-species/types";

interface PlantSpeciesCreateModalProps {
    open: boolean;
    onClose: () => void;
    onSubmitSuccess?: (data: CreatePlantSpeciesDTO) => void;
    isDark?: boolean;
}
...
export function PlantSpeciesCreateModal({
    ...
    } = useForm<CreatePlantSpeciesDTO>();

    const onSubmit = async (data: CreatePlantSpeciesDTO) => {
        if (onSubmitSuccess) {
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/40">
            <div className={`${isDark ? 'bg-gray-800 text-white' : 'bg-white text-black'} rounded-2xl shadow-2xl w-full max-w-md p-4 relative`}>
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-bold">Crear nueva especie</h2>
                    <button
